...
def parse_kw_filename(bibliometer_path, corpus_year, metric, extension, datatype):
    ...
    path_dic = _set_paths(bibliometer_path,corpus_year,datatype)
...
def _set_wordcloud_title(corpus_year, dept, kw_type, datatype, institute):
    ...
    kw_type_long = gg.KW_DICT[kw_type]
    
    title = f'Institut: {institute},Départemement: {dept}, Année: {corpus_year}\n' 
    title = title + f'Base de données : {datatype}, Mot-clé: {kw_type_long}'
...
def _set_set_marker_countries_analysis(countries,
                                       continents_df):
    
    """
    """
    publi_continent_dict = dict(zip(continents_df['Continent'],continents_df['Nombre de publications']))
    continent = [rg.COUNTRIES_CONTINENT_DICT[x] for x in countries['Pays']]
    continent_articles = [publi_continent_dict[x] for x in continent]

    text = [f'{x[0]}: {x[1]} articles ({x[2]} : {x[3]} articles)'  
              for x in zip(countries['Pays'],
                           countries['Nombre de publications'],
                           continent,
                           continent_articles)]
...
def plot_countries_analysis(corpus_year,bibliometer_path, datatype, institute):
    ...
    countries.sort_values(['Nombre de publications'], axis=0, ascending=False, inplace=True)
    countries['Code'] = countries['Pays'].map(rg.COUNTRIES_CODES_DICT)
    ...
    nbr_articles_per_country[ifx_france] = nbr_articles_per_country[1]
    ...
    data = dict(type='choropleth',
                ...
                text=_set_set_marker_countries_analysis(countries,continents_df))
...
def _builds_if_bargraph_title(kpi_dict,
    ...
    title_base  = (f"Intitut: {institute}, département:{dept}, année: {corpus_year}, database: {datatype}<br>"
...
def plot_if_analysis(institute,
    ...
    def _create_save_barchart(dept,
        ...
        barchart = _create_if_barchart(corpus_year,
                                       dept,
                                       bar_chart_if_df,
                                       if_col,
                                       dept_kpi_dict,
